    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            HStack {
                ...
                .confirmationDialog("", isPresented: $viewModel.showDialog) {
                    if !viewModel.soccerDiary.isMine {
        ...
        .sheet(isPresented: $viewModel.showNotifyDialog) {
            VStack(alignment: .leading) {
                ...
                Button {
                    // 축구 일기 신고하기 API 호출
                    viewModel.postNotifyDiary(diaryId: viewModel.soccerDiary.diaryId, request: DiaryNotifyRequest(reasonCode: viewModel.reasonCode)) { success in
                        if success {
